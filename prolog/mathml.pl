...
ml(Flags, underbrace(A, Under),
    munder([accentunder(false)], [munder([accentunder(false)], [X, mo([stretchy(true), overflow(scale)], &('UnderBrace'))]), Y])) :-
    ml(Flags, A, X),
    ml(Flags, Under, Y).
...
math(Flags, chi2ratio(A, _Df), Flags, A).
...
ml(Flags, instead_of(Err, Instead, Of), M) :-
    ...
    M = munder([accentunder(false)], [munder([accentunder(false)], [MInstead, mo([stretchy(true), overflow(scale)], &('UnderBrace'))]), MOf]).
...
ml(Flags, instead_of(Err, I, Instead, _Of, Of), M) :-
    ...
    M = munder([accentunder(false)], [munder([accentunder(false)], [MI, mo([stretchy(true), overflow(scale)], &('UnderBrace'))]), MOf]).
...
math(Flags, pwbinom(_K, Dist, N, P), Flags, fun(sub('P', "Bi"), [Dist ; [N, P]])).
...
% Normal distribution
%
math(Flags, pnorm(Z), Flags, fun('Phi', Z)).
math(Flags, unorm(Z), Flags, 1 - fun('Phi', Z)).
